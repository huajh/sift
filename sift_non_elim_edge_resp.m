function [Descriptors,KeyPoints,DoGs,Params] = sift_non_elim_edge_resp(img)
%% Reference:   
...
for i = 1:OCTAVE_NUM            
    ...
    %% remove the points close to the boundary
    %sigma0 = 1.6*2^(1/S);
    sigma = sigma0 * 2.^(LocalMax_OCT(3,:)/S);
    radius = sigma*MAGNIF_SIZE*SUBREGIONS_BINS/2;
    stay_idx = find(x-radius >=1 & x+radius <= N & ...
                    y-radius >=1 & y+radius <= M);
    LocalMax_OCT = LocalMax_OCT(:,stay_idx);
    
    %% Accurate KeyPoints Localication
    % to perform a detailed fit to the nearby data for location, scale, and
    % ratio fo principal
    KeyPoints_OCT = kp_select_non_elim_edge_resp(LocalMax_OCT,DoGs{i},threshold);    
    
    %% Orientation assignment
    KeyPoints_OCT = CalcKeypointsOrient(KeyPoints_OCT,DoGs{i},S,sigma0);
